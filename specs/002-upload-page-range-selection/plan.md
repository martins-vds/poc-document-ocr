...
| II  | Testing Standards (NON-NEGOTIABLE) | PASS   | `PageSelection` parser/validator and pipeline integration get unit tests under `tests/Models/PageSelectionTests.cs` and `tests/Services/PdfProcessorPageRangeTests.cs`. No live Azure calls. **TDD red→green is enforced**: every behavior-changing task in [tasks.md](tasks.md) is paired with a test task that MUST be committed failing before the implementation commit lands (see TDD pairing table at the top of `tasks.md`). |
